...
import os
import time
from constants import DEFAULT_ID, MICROHARD_DEFAULT_IP, MICROHARD_DEFAULT_PASSWORD, MICROHARD_IP_PREFIX, MICROHARD_USER, MONARK_ID_FILE_PATH, PAIR_STATUS_FILE_PATH
import fcntl
...
class MicrohardService:
    ...
    def _save_monark_id(self, monark_id:int):
        ...
        with open(MONARK_ID_FILE_PATH, 'w') as file:
            file.write(self.monark_id)

    def pair_monark(self, network_id: str, encryption_key: str, tx_power: int, frequency: int, monark_id:int) -> bool:
        ...
        is_success, responses= self.send_commands(ip_address=MICROHARD_DEFAULT_IP, password=MICROHARD_DEFAULT_PASSWORD, at_commands=at_commands)
    ...
    def login(self, encryption_key:str) -> bool:
        return self.send_commands(ip_address=self.monark_ip, password=encryption_key, at_commands=["AT"])
    ...
    def get_info(self, encryption_key:str) -> str:
        ...
        is_success, responses = self.send_commands(ip_address=self.monark_ip, password=encryption_key, at_commands=at_commands)
        if not is_success:
            return "No MONARK found."
        
        return f"{responses[0].strip()},{responses[1].strip()},{self.monark_id}"
    
    def change_monark_id(self, encryption_key:str, new_monark_id) -> str:
        """ 
        ...
        is_success, responses = self.send_commands(ip_address=self.monark_ip, password=encryption_key, at_commands=[at_command])
    ...
    def send_commands(self, password:str, at_commands: List[str], ip_address: str = "") -> Tuple[bool, List[str]]:
        """
            ...
            for command in at_commands:
                ...
                while time.time() < end_time:
                    if shell.recv_ready():
                        ...
                        if "OK" in part:
                            break
                        elif "ERROR" in part:  # TODO will ERROR work -> what about if no auth?
                            is_failure = True
                            break 
                    else:
                        time.sleep(0.1)
