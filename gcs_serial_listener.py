...
class GCSSerialListener:
    ...
    def _is_serial_ready(self) -> None:
        if os.path.exists(self.serial_path):
            try:
                self.serial_connection = serial.Serial(self.serial_path, BAUD_RATE, timeout=.5)
            ...
            if command_type:
                if command_type == SerialCommands.PAIR.value:
                    if os.path.exists(PAIR_STATUS_FILE_PATH):
                        ...
                        network_id, encryption_key, tx_power, frequency, monark_id = command_value.split(",")
                        # run as a thread since we need to free up the serial port for status queries
                        subprocess.run(["python", "-c", f"from microhard_service import MicrohardService; MicrohardService().pair_monark('{network_id}', '{encryption_key}', {int(tx_power)}, {int(frequency)}, {int(monark_id)})"])
                    ...
                    is_valid_creds, _ = MicrohardService().login(encryption_key=command_value)
                    if is_valid_creds:
                    ...
                    is_success, ret_msg = MicrohardService().send_commands(password=self.encryption_key, at_commands=[command])
                    # if this command was successful, we need to update the encryption key
                    if is_success:
                        self.encryption_key = command_value.split(",")[0] # it's entered twice in the value
                else:
                    # all other commands are 1:1 and don't need any manipulation
                    _, ret_msg = MicrohardService().send_commands(password=self.encryption_key, at_commands=[command])
